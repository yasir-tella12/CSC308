...
pub static mut LAST_CHAR: Option<char> = None;
        ...
        if let Some(key) = keyboard.process_keyevent(key_event) {
            match key {
                DecodedKey::Unicode(character) => match character {
                    '\u{0008}' => {
                        if let Some(frame_buffer_writer) = FRAME_BUFFER_WRITER.lock().as_mut() {
                            frame_buffer_writer.back_space();
                        }
                    }

                    _ => {
                        unsafe { LAST_CHAR = Some(character) }
                        print!("{}", character);
                    }
                },
                ...
                DecodedKey::RawKey(key) => match key {
                    KeyCode::ArrowUp => {
                        if let Some(frame_buffer_writer) = FRAME_BUFFER_WRITER.lock().as_mut() {
                            frame_buffer_writer.move_cursor_up()
                        }
                    }
                    KeyCode::ArrowDown => {
                        if let Some(frame_buffer_writer) = FRAME_BUFFER_WRITER.lock().as_mut() {
                            frame_buffer_writer.move_cursor_down()
                        }
                    }
                    KeyCode::ArrowLeft => {
                        if let Some(frame_buffer_writer) = FRAME_BUFFER_WRITER.lock().as_mut() {
                            frame_buffer_writer.move_cursor_left()
                        }
                    }
                    KeyCode::ArrowRight => {
                        if let Some(frame_buffer_writer) = FRAME_BUFFER_WRITER.lock().as_mut() {
                            frame_buffer_writer.move_cursor_right()
                        }
                    }
                    _ => {
                        print!("{:?}", key)
